...
export default async function ClientPlansPage({
  ...
  const tm = await getTranslations("mealPlans");
  const supabase = await createClient();
  const [plans, mealPlans] = await Promise.all([
    new WorkoutsService(supabase).listForClient(userId),
    new MealPlansService(supabase).listForClient(userId),
  ]);
  ...
  return (
    <div className="p-4 sm:p-6 space-y-8 max-w-4xl mx-auto">
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-medium">{tm("title")}</h2>
        {mealPlans.length === 0 ? (
          <p className="text-muted-foreground">{tm("empty")}</p>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2">
            {mealPlans.map((p) => (
              <Card key={p.id} className="p-4 space-y-2">
                ...
                <Button asChild size="sm">
                  <Link href={`/client/plans/meal/${p.id}`}>{t("viewDetails")}</Link>
                </Button>
              </Card>
            ))}
          </div>
